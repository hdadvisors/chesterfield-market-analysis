...
```{r}
#| label: fig-tenure-owner-img
#| fig-cap: "Homeowner cost burden"
...
oo_cb <- tenure_income_chas |>
  ...
  mutate(year = as.character(year),
         income = str_wrap(income, 10))

ggplot(oo_cb,
       aes(x = income, y = percent, fill = year)) +
  geom_col(position = "dodge") +
  geom_text(aes(label = label_percent(accuracy = 0.1)(percent),
                color = year,
                vjust = -.5),
            size = 3,
...
```{r}
#| label: fig-tenure-renter-img
#| fig-cap: "Renter cost burden"
...
```{r}
#| label: fig-hhtype-img
#| fig-cap: "Household type cost burden"
...
ggplot(hh_cb,
       aes(x = hhtype, y = percent, fill = year)) +
  geom_col(position = "dodge") +
  geom_text(aes(label = label_percent(accuracy = 0.1)(percent),
                color = year,
                vjust = -.5),
  ...
  scale_y_continuous(labels = label_percent(),
                     limits = c(0, 0.99)) +
  labs(title = "Household type cost burden",
       subtitle = "2012 and 2018",
       caption = "Source: HUD Comprehensive Housing Affordability Strategy.") +
  theme(axis.title = element_blank(),
        axis.ticks = element_blank(),
...
race_cb <- race_clean |>
  ...
  summarise(estimate = sum(estimate)) |>
  mutate(percent = estimate/sum(estimate)) |> 
  filter(costburden == "Cost-burdened") |> 
  mutate(year = as.character(year),
         race = gsub(" ", "\n", race))
# 
# 
ggplot(race_cb,
       aes(x = race, y = percent, fill = year)) +
  geom_col(position = "dodge") +
  geom_text(aes(label = label_percent(accuracy = 0.1)(percent),
                color = year,
                vjust = -.5),
  ...
  scale_y_continuous(labels = label_percent(),
                     limits = c(0, 0.99)) +
  labs(title = "Race and ethnicity cost burden",
       subtitle = "2012 and 2018",
       caption = "Source: HUD Comprehensive Housing Affordability Strategy.") +
  theme(axis.title = element_blank(),
        axis.ticks = element_blank(),
...
```{r}
#| label: fig-structure
#| fig-cap: "Percent of households with cost burden by structure type"
...
ggplot(cf_pums_summary,
  ...
  labs(title = "Percent of households with cost burden \nby structure type",
       caption = "Source: U.S. Census Bureau, American Community Survey, 5-year estimates. Public Use Microdata Sample.")
...
```{r}
#| label: fig-structure-sf
#| fig-cap: "Percent of households in single-family detached homes with cost burden by tenure"
...
ggplot(cf_pums_sf,
  ...
  theme(axis.title = element_blank(),
        ...
        panel.border = element_rect(color = "grey", fill = NA)) +
  scale_x_continuous(labels = percent_format(), breaks = c(.1, .2, .3, .4, .5, .6)) +
  labs(title = "Percent of households in single-family \ndetached homes with cost burden by tenure",
       caption = "Source: U.S. Census Bureau, American Community Survey, 2016-2020 5-year estimates. Public Use Microdata Sample.")
...
```{r}
#| label: fig-cb-owner
#| fig-cap: "Percent of homeowners with cost burden by tract"
...
source_acs <- tags$div(
  HTML('Source: U.S. Census Bureau, American Community Survey, 2016-2020 5-year estimates. Table B25106.')
)
...
leaflet(cf_cb_owner) %>%
  ...
  addControl(source_acs, position = "bottomright")
...
![Percent of homeowners with cost burden by tract](images/fig-cb-owner.png)
:::
...
```{r} 
#| label: fig-cb-renter
#| fig-cap: "Percent of renters with cost burden by tract"
...
::: {.content-visible when-format="pdf"}
![Percent of renters with cost burden by tract](images/fig-cb-renter.png)
:::
