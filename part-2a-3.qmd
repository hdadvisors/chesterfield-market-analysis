...
```{r}
#| label: fig-price-all
#| fig-cap: "Average rent of existing and recent rental units"
...
```{r} 
#| label: fig-price-br
#| fig-cap: "Average rent of existing and recent rental units by bedroom"
...
```{r}
#| label: fig-unit-mix
#| fig-cap: "Share of units by type for existing and recent properties"
...
style_mix_bar <- ggplot(style_mix,
                        aes(x = style,
                            y = pct,
                            fill = market)) +
  geom_col() +
  ...
  labs(title = "Share of units by type for existing and recent properties",
  caption = "Source: CoStar Group, Inc.") +
...
```{r}
#| label: fig-br-mix
#| fig-cap: "Share of units by bedrooms for existing and recent properties"
...
br_mix_bar <- ggplot(br_mix,
  ...
  facet_wrap(~market) +
  scale_y_continuous(labels = percent_format()) +
  labs(title = "Share of units by bedrooms for existing and recent properties",
  caption = "Source: CoStar Group, Inc.") +
  theme(axis.title = element_blank(),
        legend.position = "none")
...
```{r}
#| label: fig-sf-mix
#| fig-cap: "Share of units by size for existing and recent properties"
...
```{r}
#| label: fig-amenities
#| fig-cap: "Percent of existing and recent properties by number of amenities"
...
```{r}
#| label: fig-location
#| fig-cap: "Map of recent and existing multifamily rental properties"
...
comp_location_map <- leaflet(comp_map2) |>
  ...
  addLegend("topright", pal = pal, values = ~market,
            title = "Market", opacity = 1) |> 
...
![Map of recent and existing multifamily rental properties](images/fig-location.png)
:::
...
```{r}
#| label: fig-transit-chart
#| fig-cap: "Percent of units in Chesterfield County near transit by market"
...
```{r}
#| label: fig-schools-chart
#| fig-cap: "Driving time to schools in Chesterfield County by market"
...
```{r}
#| label: fig-grocery-chart
#| fig-cap: "Mean distance from apartments in Chesterfield County to food stores by market"
...
#| fig-cap: "Mean driving time from apartments in Chesterfield County to healthcare facility by market"
...
```{r}
#| label: fig-jobs-chart
#| fig-cap: "Percent of jobs by wage within 15 minute drive by market in Chesterfield County"
