...
```{r hsr-rhh, fig.cap="Renter households from 2010 to 2020 within Eastern Hull Street study area"}
...
```{r sfr-hsr, fig.cap="Percent of single-family homes that are renter-occupied by tract"}
...
```{r acs-hsr-rent, fig.cap="Median inflation-adjusted rent by tract: 2010 to 2020"}
...
```{r acs-yrblt, fig.cap="Eastern Hull Street rental housing by year built and census tract"}
...
```{r costar-hsr-3, fig.cap="Eastern Hull Street corridor area multifamily properties by year built"}
...
```{r hsr_transit-chart, fig.cap="Eastern Hull Street multifamily by distance to transit"}
...
```{r hsr-schools-chart, fig.cap="Eastern Hull Street multifamily by drive time to schools"}
...
```
```{r hsr-grocery-chart, fig.cap="Eastern Hull Street multifamily by distance to food access"}
...
```{r hospital-chart, fig.cap="Eastern Hull Street multifamily by drive time (in minutes) to healthcare"}
...
```{r hsr-jobs, fig.cap="Eastern Hull Street multifamily by number of jobs within a 15 minute drive"}
...
```{r rockwood, fig.cap= "Rockwood Square Shopping Center. Source: Areaprops.com", out.height="500px"}

knitr::include_graphics("images/rockwood-square.jpg")
...
```{r hsr-home-price, fig.cap="Income needed to afford median priced home in Eastern Hull Street area"}
...
```{r sfr-mls-bar, fig.cap="Renter income of census tracts and income needed to afford SFR and homeownership"}

x <- c("Census Tract 1002.06",
       "Census Tract 1002.09",
       "Census Tract 1002.10", "Census Tract 1002.11",
       "SFR", "Home Purchase")
y <- c(54615, 44732, 64952, 33618, 59160, 81197)
