...
As of June 2022, there were 34 existing market-rate multifamily developments that were either built in the last five years, under construction, or proposed. Thirty-two percent of these recent developments have been constructed or will be constructed in the Midlothian District. Over 56 percent of these properties are under construction or proposed --- with most of this development once again occurring mostly in the Midlothian District.

In 2023, 2,057 new rental homes are expected to be completed --- the most new multifamily homes completed in the last five years. Most recent development consists of low-rise buildings and offer luxury amenities such as a clubhouse, pool, and fitness center.
...
status_units <- newbuilds_by_district |> 
  st_drop_geometry() |> 
  select("Status" = building_status, number_of_units, "Year" = year_built)|> 
  mutate_at(vars("number_of_units"),
                 ~replace_na(.,0)) |> 
  group_by(Year) |> 
  summarise(Units = sum(number_of_units)) |> 
...
status_timeline <- gg_bar(status_units,
                              x_var = Year,
                              y_var = Units)
...
#Create bar chart for NOAH units by magisterial districts.
