...
```{r}
#| label: fig-hsr-rhh
#| fig-cap: "Renter households in Eastern Hull Street study area"
...
#   
# write_rds(b25003_r360, "data/b25003_hrs.rds")

b25003_hrs <- read_rds("data/b25003_hrs.rds")
...
hsr_renters_plot <- gg_line(
  ...
  y_breaks_n = 5,
  y_zero = TRUE,
  title = "Renter households in Eastern Hull Street study area",
...
hsr_map <- leaflet(hsr_map) |>
  addProviderTiles(providers$CartoDB.Positron) |>
  addPolygons(data = hsr_tracts,
              weight = 2,
              color = "#666666",
              fillColor = "#999999",
              fillOpacity = 0.3) |>
  addCircleMarkers(radius = 3,
                   popup = hsr_map$property_name) |> 
...
sfr_hsr_pct <- gg_hbar_col(sfr_hsr_summary,
                          x_var = percent_sfh,
                          y_var = tract,
                          col_var = percent_sfh,
                          title = "Percent of single-family homes that are renter-occupied by tract neighborhood",
...
# 
# write_rds(hsr_rent_cpi, "data/hsr_rent_cpi.rds")

hsr_rent_cpi <- read_rds("data/hsr_rent_cpi.rds")

hsr_rent_change <- hsr_rent_cpi |> 
  filter(NAMELSAD != "Census Tract 1002.11") |> 
...
```{r}
#| label: fig-costar-hsr-rent
#| fig-cap: "CoStar average asking rent for Eastern Hull Street corridor"
...
gg_line(costar_hsr_rent,
        ...
        title = "CoStar average asking rent for Eastern Hull Street corridor",
        caption = "Source: CoStar Group, Inc.") +
  ggplot2::scale_x_date(limits = c(as.Date("2011-12-01"), as.Date("2022-07-31")))
...
```{r}
#| label: fig-costar-hsr-brent
#| fig-cap: "CoStar average asking rent for Eastern Hull Street corridor by bedroom"
...
gg_line_col(costar_hsr_brent,
            x_var = period,
            y_var = rent,
            col_var = br,
            alpha_point = 0,
            size_line = 0.75,
            title = "CoStar average asking rent for Eastern Hull Street corridor by bedroom",
            y_labels = label_dollar(),
            x_labels = label_date(format = "%Y"),
...
```{r}
#| label: fig-mls-sfr
#| fig-cap: "Listed MLS rents in Eastern Hull Street Road corridor"
...
gg_smooth(mls_hsr_sfr,
         x_var = date,
         y_var = list_price,
         y_labels = label_dollar(),
         x_title = "",
         y_title = "Listed rent",
         title = "Listed MLS rents in Eastern Hull Street corridor",
         caption = "Source: CVR MLS.") +
  ggplot2::scale_x_continuous(limits = c(2019.7, 2022.55),
...
# 
# write_rds(b25036_hsr, "data/b25036_hsr.rds")

b25036_hsr <-  read_rds("data/b25036_hsr.rds")
...
b25036_hsr_grp <- b25036_hsr_summary |> 
  select(tract, yrgrp, estimate) |>
  group_by(tract) |> 
  mutate(percent = estimate/sum(estimate)) |> 
  ungroup() |> 
  mutate(yrgrp = fct_relevel(yrgrp, c("2010 or later", "2000 to 2009", "1980 to 1999")))

gg_hbar_col(b25036_hsr_grp,
            x_var = percent,
            y_var = tract,
            col_var = yrgrp,
            stack = TRUE,
            col_rev = TRUE,
            title = "Percent of rental units by year built by tract",
            caption = "Source: U.S. Census Bureau, American Community Survey, 2016-2020 5-year estimates. Table B25035.",
            col_title = "Year built",
            x_labels = label_percent(),
            y_labels = function(x) x,
            x_title = "",
            y_title = "")
...
```{r}
#| label: tbl-costar-hsr-3
#| tbl-cap: "Eastern Hull Street corridor (and nearby Midlothian corridor) multifamily properties"
...
#| tbl-cap: "Calculated income needed to rent single-family home in the Eastern Hull Street corridor"

hsr_sfr_price <- 1479
hsr_sfr_income <- ((hsr_sfr_price*10)/3)*12

sfr_labels <- c("Average rent for single-family home",
                "Income needed")

sfr_values <- formattable::currency(c(hsr_sfr_price, hsr_sfr_income))
...
#| fig-cap: "Renter income and income needed to purchase or rent single-family home in Eastern Hull Street corridor"
...
hsr_trans_sum <- hsr_trans_map |> 
  ...
  mutate(walk_to_transit = parse_number(format(round(walk_to_transit, 2))),
         property_name = str_wrap(property_name, width = 10))

gg_bar(hsr_trans_sum,
       x_var = property_name,
       y_var = walk_to_transit,
       x_title = "",
       y_title = "Miles",
       x_labels = function(x) x,
       x_reorder = TRUE,
       title = "Eastern Hull Street multifamily by distance to transit",
...
gg_bar(hsr_schools_sum,
       ...
       x_labels = function(x) x,
       x_reorder = TRUE,
       title = "Eastern Hull Street multifamily by drive time to schools",
...
hsr_grocery_sum <- hsr_grocery_map |> 
  ...
  mutate(property_name = fct_reorder(property_name, Miles, .fun = sum, .desc = TRUE))

gg_hbar_col_facet(hsr_grocery_sum,
       y_var = property_name,
       x_var = Miles,
       y_title = "",
       x_title = "Miles",
       ...
       #x_reorder = TRUE,
       col_legend_none = TRUE,
       title = "Eastern Hull Street multifamily by distance to food access",
...
```{r}
#| label: fig-hsr-health-chart
#| fig-cap: "Mean driving time from apartments in Eastern Hull Street corridor to healthcare facility"
...
hsr_urgent_sum <- hsr_urgent_map |> 
  ...
  mutate(property_name = fct_reorder(property_name, Minutes, .fun = sum, .desc = TRUE))

gg_hbar_col_facet(hsr_urgent_sum,
       y_var = property_name,
       x_var = Minutes,
       y_title = "",
       x_title = "Minutes",
       ...
       #x_reorder = TRUE,
       col_legend_none = TRUE,
       title = "Mean driving time from apartments in Eastern Hull Street corridor to healthcare facility",
...
hsr_jobs_sum <- hsr_jobs |>
  ...
  ungroup() |> 
  mutate(property_name = str_wrap(property_name, width = 10))

gg_bar_col(hsr_jobs_sum,
           x_var = property_name,
           y_var = percent,
           col_var = wage,
           stack = TRUE,
           ...
           col_title = "Annual wage",
           col_rev = TRUE,
           title = "Percent of jobs by wage within a 15 minute drive of Eastern Hull Street multifamily",
...
![Rockwood Square Shopping Center.](images/rockwood-square.jpg)
*Photo: Areaprops.com*
