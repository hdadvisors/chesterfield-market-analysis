...
```{r}
#| label: fig-r1-map
#| fig-cap: "Route 1 corridor study area"

r1_tracts_list <- c("100300", "100404", "100405", "100406", "100407", "100409")

r1_tracts <- read_rds("data/r1_tracts.rds")
...
```{r}
#| label: fig-r1-rhh
#| fig-cap: "Renter households in Route 1 corridor"
...
#   mutate(NAME = str_remove_all(NAME, ", Chesterfield County, Virginia")) |> 
#   mutate(neighborhood = case_when(
#       NAME == "Census Tract 1003" ~ "Bensley",
#       NAME == "Census Tract 1004.05" ~ "Bensley",
#       NAME == "Census Tract 1004.04" ~ "Bellwood",
#       NAME == "Census Tract 1004.06" ~ "Bellwood",
#       NAME == "Census Tract 1004.07" ~ "Bellwood",
#       NAME == "Census Tract 1004.09" ~ "Northern Chester",
#     ))
# 
# write_rds(b25003_r1, "data/b25003_r1.rds")

b25003_r1 <- read_rds("data/b25003_r1.rds")
...
r1_renters_plot <- gg_line(
  ...
  y_breaks_n = 5,
  y_zero = TRUE,
  title = "Renter households in Route 1 corridor",
...
```{r}
#| label: fig-r1-rhh-tract
#| fig-cap: "Change in renter households by tract from 2010 to 2020"

b25003_r1_10 <- b25003_r1 |> 
  filter(year == 2010) |> 
  select(NAME, fips, year, estimate, neighborhood)
...
b25003_r1_10_20 <- cbind(b25003_r1_10, b25003_r1_20) |> 
  select(NAME, fips, est10 = estimate, est20 = estimate.1, geometry, neighborhood) |> 
  mutate(change = (est20-est10),
         tract = sapply(strsplit(
           as.character(
             str_remove_all(NAME, "\\,")), " "), unlist)[3,],
         tract = paste0(tract, " (", neighborhood, ")"), 
...
```{r}
#| label: fig-costar-mf-1
#| fig-cap: "Total number of multifamily units by property status"
...
```{r}
#| label: fig-costar-mf-2
#| fig-cap: "Map of multifamily properties along Route 1 Corridor"
...
```{r}
#| label: fig-r1-mhc
#| fig-cap: "Map of manufactured home communities along Route 1"
...
source_mhccv <- tags$div(
  HTML('Source: Manufactured Home Community<br>Coalition of Virginia.')
)
...
r1_mhc_map <- leaflet(mhc_coded_r1) |>
  ...
  addControl(source_mhccv, position = "bottomright")
...
```{r}
#| label: tbl-r1-mhc-list
#| tbl-cap: "Manufactured home communities along northern Route 1 corridor"
...
```{r}
#| label: fig-sfr-r1
#| fig-cap: "Percent of single-family homes that are renter-occupied by tract and neigborhood"
...
#   filter(imprvmn > 0)
# 
# write_rds(sfr_r1, "data/sfr_r1.rds")
# 
...
#   mutate(neighborhood = case_when(
#       tract == "1003" ~ "Bensley",
#       tract == "1004.05" ~ "Bensley",
#       tract == "1004.04" ~ "Bellwood",
#       tract == "1004.06" ~ "Bellwood",
#       tract == "1004.07" ~ "Bellwood",
#       tract == "1004.09" ~ "Northern Chester",
#     )) |> 
#   mutate(tract = paste0(tract, " (", neighborhood, ")")) |> 
...
sfr_r1_pct <- gg_hbar_col(sfr_r1_summary,
                          ...
                          y_var = tract,
                          col_var = percent_sfh,
                          title = "Percent of single-family homes that are renter-occupied by tract and neighborhood",
...
```{r}
#| label: tbl-sfr-r1-owners
#| tbl-cap: "Owner location for single-family rentals along Route 1 corridor"
...
sfr_r1 <- read_rds("data/sfr_r1.rds")
...
#   ))
# 
# write_rds(r1_rent_cpi, "data/r1_rent_cpi.rds")

r1_rent_cpi <- read_rds("data/r1_rent_cpi.rds")
...
gg_line_col_facet(r1_rent_change,
                  ...
                  caption = "Source: U.S. Census Bureau, American Community Survey, 5-year estimates. Table B25064.",
...
```{r}
#| label: fig-acs-r1-rent1
#| fig-cap: "Median inflation-adjusted rent by neighborhood: 2010 to 2020"
...
```{r}
#| label: fig-costar-r1-rent
#| fig-cap: "CoStar average asking rent for Route 1 corridor"
...
```{r}
#| label: fig-costar-r1-brent
#| fig-cap: "CoStar average asking rent for Route 1 corridor by bedroom"
...
```{r}
#| label: fig-mls-sfr
#| fig-cap: "Listed MLS rents in Route 1 corridor"
...
```{r}
#| label: fig-mls-sfr1
#| fig-cap: "Percent of MLS-listed SFR by bedrooms in Route 1 corridor"
...
```{r}
#| label: fig-acs-yrblt
#| fig-cap: "Percent of rental units built before 1980 by tract"
...
#     ))
# 
# write_rds(b25036_r1, "data/b25036_r1.rds")

b25036_r1 <-  read_rds("data/b25036_r1.rds")
...
b25036_r1_grp <- b25036_r1_grp |> 
  group_by(yrgrp, neighborhood, NAME) |> 
  summarise(estimate = sum(estimate)) |> 
...
gg_hbar_col(b25036_r1_grp,
            ...
            caption = "Source: U.S. Census Bureau, American Community Survey, 2016-2020 5-year estimates. Table B25035.",
            x_labels = label_percent(),
            y_labels = function(x) x,
            col_legend_none = TRUE,
            x_title = "",
...
```{r}
#| label: fig-costar-mf-3
#| fig-cap: "Multifamily units in Route 1 corridor by year built"
...
```{r}
#| label: fig-costar-mf-4
#| fig-cap: "Percent of rental units in Route 1 corridor by CoStar star rating"
...
```{r}
#| label: fig-mhccv-condition
#| fig-cap: "Percent of MH units in Route 1 corridor by park typology"

mhc_con <- read_csv("data/mhccv_chesterfield.csv") |> 
  filter(corridor == "Route 1 - N") |> 
  group_by(typology) |> 
  summarise(units = sum(units)) |> 
  mutate(typology = gsub(" ", "\n", typology)) |> 
  mutate(pct = units/sum(units),
...
ggplot(mhc_con,
  ...
  labs(title = "Percent of MH units in Route 1 corridor by park typology",
...
intrate <- 0.0472 # Current average interest as of 9.6.2022
...
```{r}
#| label: tbl-r1-sfr-price
#| tbl-cap: "Calculated income needed to rent single-family home in the Route 1 corridor"
...
#| fig-cap: "Renter income and income needed to purchase or rent single-family home in Route 1 corridor"

x <- c("Bensley (1003)", "Bellwood (1004.04)", "Bensley (1004.05)", "Bellwood (1004.06)", "Bellwood (1004.07)", "Northern Chester (1004.09)", "Rent single-family home", "Purchase single-family home")
y <- c(35906, 32014, 39477, 25861, 64896, 63783, 53475, 71952)
...
```{r}
#| label: fig-r1-transit-chart
#| fig-cap: "Percent of units in Route 1 corridor near transit by rent type"
...
gg_bar_col_facet(r1_trans_sum,
                 ...
                 caption = "Sources: CoStar Group, Inc. and GRTC."
                 )
...
```{r}
#| label: fig-r1-schools-chart
#| fig-cap: "Driving time to schools in Route 1 corridor by rent type"
...
gg_bar_col(r1_schools_map,
           ...
           caption = "Sources: CoStar Group, Inc. and CCPS.")
...
```{r}
#| label: fig-r1-grocery-chart
#| fig-cap: "Mean distance from apartments in Route 1 corridor to food stores by type"
...
gg_bar_col_facet(r1_grocery_sum,
                 ...
                 caption = "Sources: CoStar Group, Inc. and USDA."
                 )
...
```{r}
#| label: fig-r1-health-chart
#| fig-cap: "Mean driving time from apartments in Route 1 corridor to healthcare facility by type"
...
gg_bar_col_facet(r1_health_sum,
                 ...
                 caption = "Sources: CoStar Group Inc. and U.S. Department of Homeland Security."
                 )
...
```{r}
#| label: fig-r1-jobs-chart
#| fig-cap: "Percent of jobs by wage within 15 minute drive by rent type in the Route 1 corridor"
...
gg_bar_col_facet(r1_jobs_sum,
                 ...
                 caption = "Sources: CoStar Group, Inc. and U.S. Census Bureau LEHD Origin-Destination Employment Statistics."
                 )
...
![The Jane at Moore's Lake.](images/thejane.jpg)
...
![Proposed site plan for Moore's Lake development.](images/mooreslakesite.jpg)
