...
::: callout-note
Although it has been the go-to measure in housing affordability, the 30 percent standard has several criticisms. Chief among these critiques is that living expenses vary from household to household.[^part-1a-2-2] A family of four will likely face much higher and additional costs for things like childcare, food, and transportation, when compared to a single adult with the same income.

Secondly, different income groups don't spend the same way.[^part-1a-2-3] Higher income households can have much more flexibility to spend more than 30 percent of their income on housing costs when compared to a low income household. Using the 30 percent standard in this case can sometimes overestimate the share of higher income households experiencing cost burden.
...
[^part-1a-2-2]: Herbert, Christopher, Alexander Hermann, & Daniel McCue. (2018). Measuring Housing Affordability: Assessing the 30 Percent Income Standard. September 2018. Joint Center for Housing Studies of Harvard University.

[^part-1a-2-3]: Daniel Kay Hertz. (2017). "Affordability: The 30 Percent Standard's Blinders," Shelterforce. April 25, 2017. Retrieved from https://shelterforce.org/2017/04/25/30-percent-standards-blinders/.
...
```{r}
#| label: tbl-cost-burden
#| tbl-cap: "Cost burden by tenure in Chesterfield County"
...
The table below shows the inputs and calculated HAI for Chesterfield County as of August 2022. The median sales price is collected from all 2022 year-to-date single-family home sales accessed from the Central Virginia Regional Multiple Listing Service (CVR MLS).[^part-1a-2-4] The 30-year fixed mortgage is sourced from the national weekly average published by the Freddie Mac [Primary Mortgage Market Survey](https://www.freddiemac.com/pmms) for the week ending September 1, 2022. The median family income is the latest value published by the U.S. Census Bureau.[^part-1a-2-5]

[^part-1a-2-4]: Through August 31, 2022.

[^part-1a-2-5]: U.S. Census Bureau, American Community Survey, 2016-2020 5-year estimates. Table B19113.
...
```{r}
#| label: tbl-hai
#| tbl-cap: "Housing Affordability Index in Chesterfield County"
...
```{r}
#| label: tbl-residual-cost
#| tbl-cap: "Self-Sufficiency Standard monthly costs"
...
#| tbl-cap: "Residual income available for housing for family of four at 80 percent AMI"
...
This \$363 available for housing is far below than the required monthly payment to afford the median-priced single-family home (\$1,717, from HAI section above) and the market rent for apartments with three or more bedrooms (\$1,759, via CoStar[^part-1a-2-6]) in Chesterfield County.

[^part-1a-2-6]: Current as of September, 5 2022.

For additional examples, we can estimate residual income for some of the county's most common household types and incomes[^part-1a-2-7]:

[^part-1a-2-7]: Based on 2016-2020 American Community Survey 5-year estimates, regional mean annual wages, Social Security benefit data, and Chesterfield County wage data.
...
```{r}
#| label: common-households

# Find the most common household types by tenure
...
b25115_vars_cleaned <- b25115_vars %>%
  separate(label, into = c("est", "total", "tenure", "family", "married", "hhsex", "children" ), sep = "!!") %>% 
  select(variable = name, tenure, family, married, hhsex, children) %>%
  filter(variable != "B25115_001") %>%
  filter(variable != "B25115_002") %>%
  filter(variable != "B25115_003") %>%
  filter(variable != "B25115_004") %>%
  filter(variable != "B25115_007") %>%
  filter(variable != "B25115_008") %>%
  filter(variable != "B25115_011") %>%
  filter(variable != "B25115_015") %>%
  filter(variable != "B25115_016") %>%
  filter(variable != "B25115_017") %>%
  filter(variable != "B25115_020") %>%
  filter(variable != "B25115_021") %>%
  filter(variable != "B25115_024") %>%
  mutate(tenure = case_when(
    tenure == "Owner occupied:" ~ "Homeowner",
    tenure == "Renter occupied:" ~ "Renter"
  )) %>%
  mutate(family = case_when(
    family == "Family households:" ~ "Family",
    family == "Nonfamily households" ~ "Nonfamily"
  )) %>%
  mutate(married = case_when(
    married == "Married-couple family:" ~ "Married-couple",
    married == "Other family:" ~ "Other family",
    family == "Nonfamily" ~ "Nonfamily"
  )) %>%
  mutate(hhsex = case_when(
    hhsex == "With own children of the householder under 18 years" ~ "Married-couple",
    hhsex == "No own children of the householder under 18 years" ~ "Married-couple",
    hhsex == "Male householder, no spouse present:" ~ "Single male",
    hhsex == "Female householder, no spouse present:" ~ "Single female",
    family == "Nonfamily" ~ "Nonfamily"
  )) %>%
  mutate(children = case_when(
    variable == "B25115_005" ~ "With children",
    variable == "B25115_006" ~ "No children",
    variable == "B25115_018" ~ "With children",
    variable == "B25115_019" ~ "No children",
...
b25115_data <- b25115_raw %>%
  ...
  select(NAME, year, tenure, family, married, hhsex, children, estimate, moe) %>%
  group_by(year, tenure, family, married, hhsex, children) %>%
  summarise(
...
```{r}
#| label: residual-households

# Customize SSS data with county-specific income info

sss <- read_csv("data/chesterfield_self-sufficiency-standard.csv")

common <- c("a2i0p0s0t0", "a2i0p0s2t0", "a1i0p0s0t0", "a1i1p1s0t0")

common_sss <- sss %>% 
  filter(`Family Type` %in% common) %>%
  mutate(hhincome = case_when(
    `Family Type` == "a2i0p0s0t0" ~ 32869.2,
    `Family Type` == "a2i0p0s2t0" ~ 136410,
    `Family Type` == "a1i0p0s0t0" ~ 54937,
    `Family Type` == "a1i1p1s0t0" ~ 23930
  )) %>%
  mutate(label = case_when(
    `Family Type` == "a2i0p0s0t0" ~ "Retired senior and spouse",
    `Family Type` == "a2i0p0s2t0" ~ "Registered nurse, police officer, 2 kids",
    `Family Type` == "a1i0p0s0t0" ~ "CCPS teacher",
    `Family Type` == "a1i1p1s0t0" ~ "Home health aide, 2 kids"
    )) %>%
  rowwise() %>%
  mutate(costsum = sum(c_across(`Child Care Costs`:`Child Tax Credit (-)`))) %>%
  mutate(takehome = case_when(
    `Family Type` == "a2i0p0s0t0" ~ 27938.82,
    `Family Type` == "a2i0p0s2t0" ~ 103135,
    `Family Type` == "a1i0p0s0t0" ~ 43072,
    `Family Type` == "a1i1p1s0t0" ~ 20231
    )) %>%
  mutate(monthly = takehome/12) %>%
  mutate(residual = monthly - costsum + Taxes) %>%
  mutate(taxexpend = hhincome - takehome)
...
```{r}
#| label: tbl-residual-takehome
#| tbl-cap: "Monthly take home pay and residual income for selected households"

common_sss %>%
  select(label, monthly, residual) %>% 
  mutate(across(2:3, ~ dollar(.x, accuracy = 0.01))) %>%
  arrange(desc(monthly)) %>% 
  kable(caption = "Monthly take home pay and residual income for selected households",
       align = "lrr",
       col.names = c("Household", "Take home pay", "Residual income")) %>% 
  kableExtra::kable_styling(
    bootstrap_options = c("condensed", "hover", "striped"))
